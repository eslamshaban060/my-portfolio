# 🚀 Eslam Shaban – Portfolio

This is my personal portfolio website built using modern web technologies.
It showcases my skills, projects, and professional experience as a Front-End Developer.

---

## 🛠 Tech Stack

- **Next.js**
- **TypeScript**
- **Tailwind CSS**
- Responsive Design
- Performance Optimization
- SEO Best Practices

---

## 📌 About The Project

This portfolio represents my approach to clean architecture, scalable structure, and modern UI development.
The goal was to build a fast, responsive, and well-structured application that reflects production-level standards.

---

## ⚙️ Getting Started

To run the project locally:

```bash
npm install
npm run dev
```

Then open:

```
http://localhost:3000
```

---

## 🌍 Live Website

👉 (Add your deployed link here)

---

## 📬 Contact Me

- 📧 Email: [eslamshaban060@gmail.com](mailto:eslamshaban060@gmail.com)
- 💼 LinkedIn: [https://www.linkedin.com/in/eslamshaban060/](https://www.linkedin.com/in/eslamshaban060/)
- 🐙 GitHub: [https://github.com/eslamshaban060](https://github.com/eslamshaban060)
- 💬 WhatsApp: [https://wa.me/201006407387](https://wa.me/201006407387)

---

## 🎯 Purpose

This portfolio is designed to present my technical skills, development style, and ability to build modern web applications using scalable and maintainable architecture.
